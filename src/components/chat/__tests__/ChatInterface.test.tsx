...
vi.mock("../MessageList", () => ({
  ...
  MessageListEmptyState: () => <div data-testid="message-list-empty-state" />,
}));
...
test("renders chat interface with message list and input", () => {
  ...
  expect(screen.getByTestId("message-list-empty-state")).toBeDefined();
...
test("renders with correct layout classes", () => {
  ...
  const emptyStateContainer = screen.getByTestId("message-list-empty-state").closest(".flex-1");
  expect(emptyStateContainer?.className).toContain("flex");
